...
namespace arm_hardware
    ...
    hardware_interface::CallbackReturn
...

        {
            ...
            auto get_string_parameter = [&](const char* parameter_name, const std::string& default_value = "") {
                auto parameter_iterator = info_.hardware_parameters.find(parameter_name);
                return parameter_iterator != info_.hardware_parameters.end() ? parameter_iterator->second : default_value;
            };
            
            auto get_unsigned_parameter = [&](const char* parameter_name, unsigned default_value) {
                auto string_value = get_string_parameter(parameter_name);
                try { 
                    return string_value.empty() ? default_value : static_cast<unsigned>(std::stoul(string_value)); 
                }
                catch (...) { 
            ...
            auto get_double_parameter = [&](const char* parameter_name, double default_value) {
                auto string_value = get_string_parameter(parameter_name);
                try { 
                    return string_value.empty() ? default_value : std::stod(string_value); 
                }
                catch (...) { 
                    return default_value; 
                }
            ...
            auto get_boolean_parameter = [&](const char* parameter_name, bool default_value) {
                auto string_value = get_string_parameter(parameter_name);
                if (string_value == "1" || string_value == "true" || string_value == "True") {
                    return true;
                }
                if (string_value == "0" || string_value == "false" || string_value == "False") {
                    return false;
                }
                return default_value;
            };

            const std::string serial_port           = get_string_parameter("port", "/dev/ttyACM0");
            const unsigned    baudrate              = get_unsigned_parameter("baudrate", 57600);
            const uint8_t     motor_id_1           = static_cast<uint8_t>(get_unsigned_parameter("motor_id_1", 1));
            const uint8_t     motor_id_2           = static_cast<uint8_t>(get_unsigned_parameter("motor_id_2", 2));
            const uint8_t     motor_id_3           = static_cast<uint8_t>(get_unsigned_parameter("motor_id_3", 3));
            const uint8_t     motor_id_4           = static_cast<uint8_t>(get_unsigned_parameter("motor_id_4", 4));
            const uint8_t     motor_id_5           = static_cast<uint8_t>(get_unsigned_parameter("motor_id_5", 5));
            const double      steps_per_revolution = get_double_parameter("steps_per_rev", 200.0);
            const bool        enable_hex_dump      = get_boolean_parameter("hex_dump", false);
            const bool        enable_probe         = get_boolean_parameter("probe", false);
            const std::string protocol_type        = get_string_parameter("protocol", "register");

            // Build the config-only bundle
            sys_ = std::make_shared<stepper::System>(
                    stepper::StepperDriver::Prepare(
                        serial_port, baudrate, motor_id_1, motor_id_2, motor_id_3, motor_id_4, motor_id_5, 
                        steps_per_revolution, enable_hex_dump, enable_probe, protocol_type));

            RCLCPP_INFO(get_logger(),
                "Prepared Stepper System: device='%s' baud=%u motor_ids=[%u,%u,%u,%u,%u] steps_per_rev=%.1f hex_dump=%d probe=%d protocol=%s",
