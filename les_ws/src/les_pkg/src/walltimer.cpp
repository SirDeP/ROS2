...
class test_walltimer : public rclcpp::Node
{
public:
    //-- constuctor:
    test_walltimer() : Node("template_walltimer_node") /*constructor = multiple inheritance*/
    {
        //--communication and timer objects:
        timer_template_ = this->create_wall_timer(std::chrono::seconds(10), std::bind(&test_walltimer::get_time, this));
        //--customs functions:
        //...
    }
...
int main(int argc, char *argv[])
{
    ...
    auto node = std::make_shared<test_walltimer>();
    rclcpp::spin(node);
    rclcpp::shutdown();
